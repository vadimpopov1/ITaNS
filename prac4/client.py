...
def switch_action(choice):
    ...
    match choice:
        ...
        case 6:
            ...
            while book_id <= 0:
                print("Номер книги должен быть не меньше 1.")
                book_id = int(input("ID: "))
            title = input("Title: ")
            author = input("Author: ")
            while any(char.isdigit() for char in author):
                print("Автор не может содержать цифры.")
                author = input("Author: ")
            publication_year = int(input("Publication year: "))
            while publication_year <= 999:
                print("Укажите верную дату публикации.")
                publication_year = int(input("Publication year: "))
